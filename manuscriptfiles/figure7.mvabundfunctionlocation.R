# Reading the data:
allspongedna=read.csv("revisedfunctions.swissprotkeywords.4mvabund.location.csv");
dim(allspongedna)
# That told me 10 rows and 4442 columns, which makes sense cause I have the 4439 columns of data and the additional columns of bubbleornot and location. 
allspongedna[,2:806]
# That's bubbleornot and location
library(vegan)
# Rarefying the data:
...
modelallspongedna=manyglm(yallspongednarare~location,data=xallspongedna)
...
anovaunipunadj_pvalue=anovaunipunadj["location",]
...
library(pheatmap)
# make a heatmap with the log10 transformed data to visualise it better:
pheatmap(log10(allspongednarare_pvalueless005_df+1), cluster_rows=F, cluster_cols=T, fontsize=6)
...
# and then this is Shaun's trick to see the standard deviation between "seep" and "control" instead of the rarefied counts:
pheatmap((allspongednarare_pvalueless005_df+1), cluster_rows=T, cluster_cols=T, fontsize=6)
